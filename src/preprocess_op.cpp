...
// 若图片宽高比大于预设值，调整为 [imgH， imgH * wh_ratio]
// 若图片宽高比小于预设值，调整为 [imgH， imgH * wh_ratio]，宽度不足，在右侧进行填充
...
// 以下的逻辑和上面等效
//  if (ceilf(imgH * ratio) > imgW){
//      cv::resize(img, resize_img, cv::Size(imgW, imgH));
//  }
//  else
//  {
//      cv::resize(img, resize_img, cv::Size(int(ceilf(imgH * ratio)), imgH), 0.f, 0.f, cv::INTER_LINEAR);
